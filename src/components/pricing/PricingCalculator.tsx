...
export default function PricingCalculator() {
  ...
  // Extract promocode from URL parameters
  useEffect(() => {
    ...
    <div className="container mx-auto justify-center flex">
      <div className="bg-white rounded-3xl shadow-(--card-shadow) p-8 max-w-[468px] w-full  border border-ink-light min-h-[610px]">
        ...
        <div
          ...
          onClick={() =>
            setLocationType(locationType === "one" ? "multiple" : "one")
          }
        >
          <div className="relative mx-auto h-12 w-full max-w-[400px]">
            ...
            >
              {locationType === "one" ? "Multiple Locations" : "One Location"}
            </div>
            <div
        ...
        {locationType === "multiple" ? (
          ...
          <>
            ...
            <div className="text-center mb-8">
              <h2 className="pay-text text-2xl font-medium text-charcoal mb-6">
                Here's what you'll pay:
              </h2>
              <div className="mb-1 flex items-center justify-center">
                <span className="text-7xl font-bold text-charcoal py-0">
                  <span className="text-2xl align-top text-charcoal">$</span>
                  {totalPrice.toFixed(2)}
                </span>
              </div>
              <p className="text-ink-dark text-lg">per month</p>
              <p className="text-ink-dark/70 text-sm mt-2">
                {getCalendarText(teamSize)}
              </p>
            ...
            <div className="flex justify-center pb-2">
              <span className="text-ink-dark/50 text-sm">
                *Cancel anytime with no cancellation fees
              </span>
            </div>
            <div className="flex justify-center pb-4">
              <span className="text-ink-dark/50 text-sm">Exclusions apply</span>
            </div>
            {/* Action Buttons */}
            <div className="space-y-4 flex flex-col items-center justify-center gap-2">
              ...
              <a
                ...
                id="contact-sales-team"
                target="_blank"
                rel="noopener noreferrer"
                className="w-full text-bluelink hover:text-blue-dark font-medium flex items-center justify-center gap-2 text-base transition-colors duration-200 cursor-pointer"
              >
                Contact Sales
                <svg
                  className="w-4 h-4"
